...
def update():
    ...
    # Find Zevah area and Replace
    target_area_id = "zevah"
    found = False
    
    for i, area in enumerate(data.get('areas', [])):
        if area.get('id') == target_area_id:
            data['areas'][i] = NEW_ZEVAH_DATA
            print(f"✅ Updated '{target_area_id}' with detailed payload.")
            found = True
            break
    ...
    if not found:
        ...
        print(f"✅ Created new area '{target_area_id}'.")
